...
module Target.ScriptContextBuilder (
  UnitTestArgs (..),
  InputBuilder (..),
  TxOutBuilder (..),
...
instance PlutusTx.Eq.Eq ScriptPurpose where
  (==) a b = PlutusTx.toBuiltinData a PlutusTx.Eq.== PlutusTx.toBuiltinData b
...
withSigner pkh = ScriptContextBuilder $ \scb ->
  scb{scbSignatories = insertSorted pkh (scbSignatories scb)}
 where
